...
public struct AuthenticatorData {
    ...
    public static func fromBytes(_ bytes: [UInt8]) -> Optional<AuthenticatorData> {
        FidoLogger.debug("<AuthenticatorData> fromBytes")
        if bytes.count < 37 {
            FidoLogger.debug("<AuthenticatorData> byte-size is not enough")
            return nil
        }
        ...
        FidoLogger.debug("<AuthenticatorData> sing-count:\(signCount)")
        ...
        if flags.hasAttestedCredentialData {
            ...
            guard let (coseKey, readSize) = COSEKeyParser.parse(bytes: rest) else {
                FidoLogger.debug("<AuthenticatorData> failed to parse COSE_Key")
                return nil
            }
        ...
        if flags.hasExtension {
            ...
            guard let params = CBORReader(bytes: rest).readStringKeyMap() else {
                FidoLogger.debug("<AuthenticatorData> failed to read CBOR for extensions")
                return nil
            }
    ...
    public func toBytes() -> [UInt8] {
        FidoLogger.debug("<AuthenticatorData> toBytes")
...
public struct AuthenticatorDataFlags {
    ...
    init(flags: UInt8) {
        ...
        FidoLogger.debug("<AuthenticatorDataFlags> UP:\(userPresent)")
        FidoLogger.debug("<AuthenticatorDataFlags> UV:\(userVerified)")
        FidoLogger.debug("<AuthenticatorDataFlags> AT:\(hasAttestedCredentialData)")
        FidoLogger.debug("<AuthenticatorDataFlags> ED:\(hasExtension)")
    }

    public func toByte() -> UInt8 {
        var flags: UInt8 = 0b00000000
        if self.userPresent {
            FidoLogger.debug("<AuthenticatorDataFlags> UP:on")
            flags = flags | UPMask
        }
        if self.userVerified {
            FidoLogger.debug("<AuthenticatorDataFlags> UV:on")
            flags = flags | UVMask
        }
        if self.hasAttestedCredentialData {
            FidoLogger.debug("<AuthenticatorDataFlags> AT:on")
            flags = flags | ATMask
        }
        if self.hasExtension {
            FidoLogger.debug("<AuthenticatorDataFlags> ED on")
            flags = flags | EDMask
        }
        return flags
    }
